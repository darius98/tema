...
TEST_CASE("algorithms.print_utf8") {
    ...
    test("On expressions", [] {
        ...
        std::stringstream sout;
        print_utf8_to(x_union_y_expr.get(), sout);
        expect(!sout.fail());
        expect(!sout.bad());
        expect(sout.str(), "X∪Y");
    });
